...
function TaskCard({ task, onEdit, onDelete, onStatusChange }) {
  ...
  return (
    <div style={{
      background: '#0d0d1a',
      border: `1px solid ${isOverdue ? 'rgba(248,113,113,0.25)' : '#1a1a3e'}`,
      borderRadius: '10px', padding: '0.9rem 1rem',
      transition: 'border-color 0.2s',
    }}
      onMouseEnter={e => e.currentTarget.style.borderColor = isOverdue ? 'rgba(248,113,113,0.45)' : 'rgba(0,180,255,0.2)'}
      onMouseLeave={e => e.currentTarget.style.borderColor = isOverdue ? 'rgba(248,113,113,0.25)' : '#1a1a3e'}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
            <h3 style={{ color: '#e0e0ff', fontSize: '0.88rem', fontWeight: 500, margin: 0, lineHeight: 1.4 }}>
              {task.title}
            </h3>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
      ...
      <button onClick={onNewTask} style={{
        ...
        padding: '0.7rem', color: '#00b4ff', fontSize: '0.85rem', fontWeight: 600,
      ...
      {/* Stats */}
      <div style={{ background: '#0d0d1a', border: '1px solid #1a1a3e', borderRadius: '12px', padding: '1rem' }}>
        <p style={{ color: '#4a4a7a', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.75rem' }}>Overview</p>
        ...
        {[
          ...
          <div key={s.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
              ...
              <span style={{ color: '#4a4a7a', fontSize: '0.78rem' }}>{s.label}</span>
            </div>
        ...
        {total > 0 && (
          <div style={{ marginTop: '0.85rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
              <span style={{ fontSize: '0.68rem', color: '#4a4a7a' }}>Progress</span>
              <span style={{ fontSize: '0.68rem', color: '#10b981', fontWeight: 600 }}>{donePercent}%</span>
            </div>
      ...
      {/* Upcoming due dates */}
      <div style={{ background: '#0d0d1a', border: '1px solid #1a1a3e', borderRadius: '12px', overflow: 'hidden' }}>
        <div style={{ padding: '0.85rem 1rem', borderBottom: '1px solid #1a1a3e' }}>
          <p style={{ color: '#4a4a7a', fontSize: '0.7rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>Due soon</p>
        </div>

        {upcoming.length === 0 ? (
          <p style={{ color: '#4a4a7a', fontSize: '0.78rem', textAlign: 'center', padding: '1.25rem', margin: 0 }}>
            ...
            {upcoming.map(t => {
              ...
              return (
                <div key={t.id} style={{
                  ...
                  border: `1px solid ${isOverdue ? 'rgba(248,113,113,0.2)' : '#1a1a3e'}`,
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.4rem' }}>
                    <span style={{
                      color: '#e0e0ff', fontSize: '0.78rem', fontWeight: 500,
                      overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1,
                    }}>{t.title}</span>
                    <span style={{
                      fontSize: '0.62rem', fontWeight: 700, flexShrink: 0,
                      color: isOverdue ? '#f87171' : isUrgent ? '#f59e0b' : '#4a4a7a',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginTop: '0.25rem' }}>
                    ...
                    <span style={{ color: '#4a4a7a', fontSize: '0.68rem' }}>{fmtDate(t.due_date)}</span>
                  </div>
                </div>
              )
            })}
...
export default function TaskTrackerPage() {
  ...
  const SectionHeader = ({ label, color, count }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '0.6rem', marginTop: '0.25rem' }}>
      ...
      <span style={{ color: '#e0e0ff', fontSize: '0.8rem', fontWeight: 600 }}>{label}</span>
      <span style={{
          ...
          <div>
            ...
            {tasksLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '3.5rem 2rem', background: '#0d0d1a', border: '1px solid #1a1a3e', borderRadius: '12px' }}>
                ...
                <p style={{ color: '#4a4a7a', fontSize: '0.85rem', margin: '0 0 0.5rem' }}>
                  {tasks.filter(t => t.status !== 'done').length === 0
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                {todo.length > 0 && (
                  <div>
                    <SectionHeader label="To Do" color="#4a4a7a" count={todo.length}/>
                ...
                {inProgress.length > 0 && (
                  <div>
                    <SectionHeader label="In Progress" color="#f59e0b" count={inProgress.length}/>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                      {inProgress.map(task => (
                        <TaskCard key={task.id} task={task} onEdit={setEditTask} onDelete={handleDelete} onStatusChange={handleStatusChange}/>
                      ))}
