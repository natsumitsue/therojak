...
const TABS = [
  ...
  { key: 'whois', label: 'Whois',        icon: '📋', placeholder: 'e.g. google.com' },
...
const SSL_GRADES = {
  ...
  'T':  { color: '#f87171', bg: 'rgba(248,113,113,0.12)' },
  'M':  { color: '#f87171', bg: 'rgba(248,113,113,0.12)' },
}
...
function ResultCard({ children }) {
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', marginTop: '1rem' }}>
      {children}
    </div>
  )
}
...
function SectionHeader({ title }) {
  return (
    <div style={{ padding: '0.6rem 1rem', background: 'rgba(128,128,128,0.04)', borderBottom: '1px solid var(--border)' }}>
...
function ErrorBox({ message }) {
  return (
    <div style={{ background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: '10px', padding: '0.85rem 1rem', color: 'var(--danger)', fontSize: '0.85rem', marginTop: '1rem' }}>
      ⚠ {message}
    </div>
  )
}
...
function InfoBox({ message, color = 'var(--accent)' }) {
  return (
    <div style={{ background: `${color}10`, border: `1px solid ${color}25`, borderRadius: '10px', padding: '0.85rem 1rem', color, fontSize: '0.82rem', marginTop: '1rem' }}>
      ℹ {message}
    </div>
  )
}
...
function IPResult({ data }) {
  ...
  return (
    <ResultCard>
      ...
      <ResultRow label="ZIP" value={data.zip}/>
      ...
      {data.abuseScore !== null && (
        <ResultRow label="Abuse Score" value={`${data.abuseScore}%`} valueColor={data.abuseScore > 25 ? 'var(--danger)' : data.abuseScore > 5 ? 'var(--warning)' : 'var(--success)'}/>
      )}
      {data.lat && data.lon && (
        <>
          <SectionHeader title="Geolocation"/>
          <ResultRow label="Coordinates" value={`${data.lat}, ${data.lon}`} mono/>
        </>
      )}
    </ResultCard>
  )
    ...
    <ResultCard>
      {hasAnswers ? (
        ...
        <div style={{ padding: '1.5rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
          No {data.type} records found for {data.domain}
        </div>
      )}
...
function WhoisResult({ data }) {
  ...
  return (
    <ResultCard>
      ...
      {data.registrantCountry && <ResultRow label="Reg. Country" value={data.registrantCountry}/>}
      {data.status?.length > 0 && (
        <>
          <SectionHeader title="Status"/>
          {data.status.map((s, i) => (
            <div key={i} style={{ padding: '0.5rem 1rem', borderBottom: '1px solid var(--border)' }}>
              <span style={{ color: 'var(--text-primary)', fontSize: '0.78rem', fontFamily: 'monospace' }}>{s}</span>
            </div>
          ))}
        </>
      )}
      {data.nameservers?.length > 0 && (
        <>
          <SectionHeader title="Name Servers"/>
          {data.nameservers.map((ns, i) => (
            <div key={i} style={{ padding: '0.5rem 1rem', borderBottom: '1px solid var(--border)' }}>
              <span style={{ color: 'var(--text-primary)', fontSize: '0.78rem', fontFamily: 'monospace' }}>{ns}</span>
            </div>
          ))}
        </>
      )}
...
function SSLResult({ data }) {
  ...
  return (
    <ResultCard>
      {data.grade && (
        <div style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1.25rem', borderBottom: '1px solid var(--border)' }}>
          <div style={{
            ...
            background: gradeStyle.bg, border: `2px solid ${gradeStyle.color}40`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '1.8rem', fontWeight: 800, color: gradeStyle.color, flexShrink: 0,
          }}>{data.grade}</div>
          <div>
            <div style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.9rem' }}>{data.domain}</div>
            <div style={{ color: gradeStyle.color, fontSize: '0.8rem', marginTop: '2px' }}>
              SSL Grade: {data.grade}
              {data.hasWarnings ? ' · Has warnings' : ''}
              {data.isExceptional ? ' · Exceptional' : ''}
            </div>
      ...
      <ResultRow label="Server Name" value={data.serverName}/>
      <ResultRow label="Has Warnings" value={data.hasWarnings} valueColor={data.hasWarnings ? 'var(--warning)' : 'var(--success)'}/>
      {data.notBefore && <ResultRow label="Valid From" value={new Date(data.notBefore).toLocaleDateString('en-MY')}/>}
      {data.notAfter && (
        <ResultRow label="Valid Until"
          value={`${new Date(data.notAfter).toLocaleDateString('en-MY')} (${Math.ceil((new Date(data.notAfter) - new Date()) / 86400000)} days)`}
          valueColor={Math.ceil((new Date(data.notAfter) - new Date()) / 86400000) < 30 ? 'var(--warning)' : 'var(--success)'}
        />
      )}
        ...
        {data.ports?.map((p, i) => (
          <div key={i} style={{
            ...
            <span style={{
              ...
              background: p.open === true ? 'rgba(16,185,129,0.12)' : p.open === false ? 'rgba(248,113,113,0.1)' : 'rgba(128,128,128,0.08)',
              color: p.open === true ? 'var(--success)' : p.open === false ? 'var(--danger)' : 'var(--text-muted)',
            }}>
    ...
    <ResultCard>
      <div style={{ padding: '1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <div style={{
          ...
          background: isReachable ? 'rgba(16,185,129,0.12)' : 'rgba(248,113,113,0.12)',
          border: `2px solid ${isReachable ? 'rgba(16,185,129,0.4)' : 'rgba(248,113,113,0.4)'}`,
        ...
        <div>
          ...
          <div style={{ color: isReachable ? 'var(--success)' : 'var(--danger)', fontSize: '0.8rem' }}>
            {isReachable ? `Reachable${data.latency ? ` · ${data.latency}ms` : ''}` : 'Not reachable'}
          </div>
        </div>
      </div>
      <SectionHeader title="Connection Results"/>
      {data.https?.reachable !== undefined && (
        <ResultRow label="HTTPS" value={data.https.reachable ? `Reachable (${data.https.ms}ms, HTTP ${data.https.status})` : 'Not reachable'} valueColor={data.https.reachable ? 'var(--success)' : 'var(--danger)'}/>
      )}
      {data.http?.reachable !== undefined && (
        <ResultRow label="HTTP" value={data.http.reachable ? `Reachable (${data.http.ms}ms, HTTP ${data.http.status})` : 'Not reachable'} valueColor={data.http.reachable ? 'var(--success)' : 'var(--danger)'}/>
      )}
      {data.dns && (
        <ResultRow label="DNS Resolved" value={data.dns.resolved ? `Yes — ${data.dns.ips?.join(', ')}` : 'No'} valueColor={data.dns.resolved ? 'var(--success)' : 'var(--danger)'}/>
      )}
      <div style={{ padding: '0.6rem 1rem' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>ℹ {data.note}</span>
      </div>
    </ResultCard>
  )
}
...
export default function NetworkPage() {
  ...
  const handleSearch = useCallback(async (q = query) => {
    ...
    try {
      ...
      if (trimmed) {
        setHistory(prev => {
          const entry = { tool: activeTab, query: trimmed, time: new Date() }
          return [entry, ...prev.filter(h => !(h.tool === activeTab && h.query === trimmed))].slice(0, 10)
        })
      }
    } catch (e) {
      setError(e.message)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', fontFamily: 'sans-serif' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 10, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '0 1.25rem', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <main style={{ position: 'relative', zIndex: 1, maxWidth: '860px', margin: '0 auto', padding: '1.5rem 1.25rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', marginBottom: '1.25rem', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '4px' }}>
          {TABS.map(t => (
            <button key={t.key} onClick={() => handleTabChange(t.key)} style={{
              flex: 1, minWidth: '100px', padding: '0.5rem 0.75rem', borderRadius: '7px', fontSize: '0.78rem',
        ...
        <div style={{ display: 'flex', gap: '0.65rem', marginBottom: '0.5rem' }}>
          ...
          {/* DNS type selector */}
          {activeTab === 'dns' && (
            <select value={dnsType} onChange={e => setDnsType(e.target.value)} style={{
              background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px',
              padding: '0.75rem 0.9rem', color: 'var(--accent)', fontSize: '0.85rem',
              outline: 'none', cursor: 'pointer', fontFamily: 'sans-serif', fontWeight: 600,
        ...
        {activeTab === 'ip' && !query && (
          <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', margin: '0 0 0.5rem' }}>
            Leave blank to check your own IP address
          </p>
        )}
        ...
        {history.filter(h => h.tool === activeTab).length > 0 && !result && !loading && (
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            ...
            {history.filter(h => h.tool === activeTab).slice(0, 5).map((h, i) => (
              <button key={i} onClick={() => { setQuery(h.query); handleSearch(h.query) }} style={{
                background: 'rgba(128,128,128,0.06)', border: '1px solid var(--border)',
                borderRadius: '99px', padding: '2px 10px', fontSize: '0.72rem',
        ...
        {/* Port checker note */}
        {activeTab === 'port' && !result && !loading && (
          <InfoBox message="Port checking uses portchecker.io API. Results show if ports are accessible from the public internet, not from your local network."/>
        )}

        {/* SSL note */}
        {activeTab === 'ssl' && !result && !loading && (
          <InfoBox message="SSL check powered by SSL Labs. First check may take 60-90 seconds to analyze — subsequent checks are faster."/>
        )}
      </main>
