  ...
  return (
    <main style={{background: 'var(--bg-base)', minHeight: '100vh', fontFamily: 'sans-serif'}}>
      ...
      <section style={{maxWidth: '600px', margin: '0 auto', padding: '60px 24px'}}>
        <h1 style={{color: '#fff', fontSize: '32px', fontWeight: '500', margin: '0 0 8px', textAlign: 'center'}}>Password <span style={{color: 'var(--accent)'}}>Generator</span></h1>
        <p style={{color: 'var(--text-muted)', textAlign: 'center', marginBottom: '40px'}}>Generate strong passwords instantly. Free, no signup.</p>

        <div style={{background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px', marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px'}}>
          <span style={{color: '#fff', fontSize: '18px', fontFamily: 'monospace', wordBreak: 'break-all', flex: 1}}>{password || 'Click generate...'}</span>
          {password && (
            <button onClick={copy} style={{background: copied ? 'var(--accent)22' : 'transparent', border: '1px solid var(--border)', borderRadius: '8px', padding: '8px 16px', color: copied ? 'var(--accent)' : 'var(--text-muted)', cursor: 'pointer', fontSize: '13px', whiteSpace: 'nowrap'}}>
              {copied ? 'Copied!' : 'Copy'}
            </button>
          )}
        </div>
        ...
        {password && (
          <div style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px'}}>
            <span style={{color: 'var(--text-muted)', fontSize: '13px'}}>Strength:</span>
        ...
        <div style={{background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px', marginBottom: '16px'}}>
          <div style={{marginBottom: '20px'}}>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '8px'}}>
              <span style={{color: 'var(--text-primary)', fontSize: '14px'}}>Length</span>
              <span style={{color: 'var(--accent)', fontSize: '14px', fontWeight: '500'}}>{length}</span>
            </div>
          ...
          {[
            ...
            <div key={opt.label} onClick={() => toggle(opt.val, opt.set)} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderTop: '1px solid var(--border)', cursor: 'pointer'}}>
              <span style={{color: 'var(--text-primary)', fontSize: '14px'}}>{opt.label}</span>
              <div style={{width: '40px', height: '22px', borderRadius: '11px', background: opt.val ? 'var(--accent)' : 'var(--border)', position: 'relative', transition: 'background 0.2s'}}>
        ...
        <button onClick={generate} style={{width: '100%', padding: '14px', background: 'var(--accent)', border: 'none', borderRadius: '8px', color: '#000', fontSize: '15px', fontWeight: '600', cursor: 'pointer'}}>
          Generate Password
        </button>
      </section>
    </main>
